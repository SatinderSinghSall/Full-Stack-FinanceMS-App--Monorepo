 ...
 *
 * Uses the existing Resend batch email service.
 */
const sendFinancialTipToAllUsers = async ({
  ...
  for (let i = 0; i < users.length; i += batchSize) {
    ...
    try {
      /*
       * Send this entire batch through the existing Resend service.
       */
      const result = await sendBatchEmails(
        batchEmails,
        `financial-tip-${Date.now()}-${i}`,
      );

      /*
       * Resend may return an error object instead of throwing.
       * Make sure those failures are counted correctly.
       */
      if (result?.error) {
        throw new Error(result.error.message || "Resend batch email failed");
      }
      ...
      console.log(
        `Financial tip email batch sent successfully: ${batch.length} emails.`,
      );
    } catch (error) {
      /*
       * If the whole Resend batch fails,
       * mark every email in this batch as failed.
       */
      batch.forEach((user) => {
        results.push({
          status: "rejected",
      ...
      console.error(
        `Financial tip email batch failed for batch starting at index ${i}:`,
        error,
      );
    }

    /*
     * Small delay between Resend batches.
     * This prevents sending all batches back-to-back.
     */
  ...
  /*
   * Log individual failed recipients.
   */
  if (failed > 0) {
    results.forEach((result) => {
      if (result.status === "rejected") {
        console.error(
          `Financial tip email failed for ${result.email}:`,
          result.reason,
        );
      }
    });
